...
contract AnonymousCryptoContract {
  ...
  // Retrieve the commitment hash for a voters vote.
   ...
   function commitToVote(uint[50] params, uint[10] yG, uint[20] xGyG, uint[100] ab) returns (bytes32[6] h) {
    uint i=0;
    uint j=0;
    uint[10] memory temp1;
    uint[20] memory temp2;
    h[0] =  sha256(msg.sender, yG, xGyG);
    for (i=0; i<5; i++) {
        for (j=0; j <10; j++) {
            temp1[j] = params[i*10 + j];
        }
        for (j=0; j<20; j++) {
            temp2[j] = ab[i*20 + j];
        }
  ...
  // r = v - xz (mod p);
  // return(r,vG)
  function createZKP(uint[5] x, uint[5] v, uint[2][5] xG) returns (uint[4][5] res) {
  ...
  function verifyZKP(uint[2][5] xG, uint[5] r, uint[3][5] vG) returns (bool){
      ...
      /*if(rGxcG[0] == vG[i*3+0] && rGxcG[1] == vG[i*3+1]) {*/
         if (i==4) {
             return true;
         }
      } else {
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPVote(uint[10] xG, uint[10] yG, uint[20] xGyG, uint[5] w, uint[5] r1, uint[5] d1, uint[5] x, uint[6] choice_all) returns (uint[20] res, uint[10] res2) {
      uint i=0;
      uint j=0;
      uint[20] memory k;
      uint[2] memory temp2;
      ...
      if (i == 4) {
          ...
          temp2[0] = G[0];
          temp2[1] = pp - G[1];
          temp1 = Secp256k1_noconflict._mul((uint)(5-i),temp2);
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPv1Vote(uint[10] xG, uint[10] yG, uint[5] w, uint[5] r1, uint[5] d1, uint[5] x, uint[6] choice_all) returns (uint[20] res, uint[20] xGyG,  uint[10] res2) {
      uint i=0;
      uint j=0;
      uint[20] memory k;
      uint[2] memory temp2;
      ...
      temp2[0] = G[0];
      temp2[1] = pp - G[1];
      temp1 = Secp256k1_noconflict._mul(5,temp2);
      ECCMath_noconflict.toZ1(temp1, pp);
      k[2] = temp1[0];
      k[3] = temp1[1];
