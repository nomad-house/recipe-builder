...
contract AnonymousVoteContract is owned {
  ...
  struct Voter {
      address addr;
      uint[2][3] registeredkey;
      uint[2][3] reconstructedkey;
      bytes32[4] commitment;
      uint[2][3] vote;
      uint       sendtrx;
  }

  // Work around function to fetch details about a voter
  function getVoter() returns (uint[6] _registeredkey, uint[6] _reconstructedkey, bytes32[4] _commitment){
  ...
  uint[2][3] public finaltally; // Final tally
  ...
  function deadlinePassed() returns (bool){
      ...
      if(state == State.FINISHED && msg.sender == owner && refunds[owner] == 0 && (block.timestamp > endRefundPhase || totaltorefund == totalrefunded)) {
         ...
         uint[2][3] memory empty;
         bytes32[4] memory empty1;
  ...
  function register(uint[2][3] xG, uint[3][3] vG, uint[3] r) inState(State.REGISTRATION) payable returns (bool) {
   ...
   */
  function submitCommitment(bytes32[4] h) inState(State.COMMITMENT) {
...
function submitVote(uint[12] ab, uint[12] xGyG, uint[6] params, uint comp) inState(State.VOTE) returns (bool) {
     // HARD DEADLINE
  ...
  function computeTally(uint comp) inState(State.VOTE) onlyOwner {
     ...
     if(temp[0] == 0) {
       ...
       uint totalregistered_1 = 3 * totalregistered;
  ...
  function verifyZKP(uint[2][3] xG, uint[3] r, uint[3][3] vG) returns (bool){
      uint[2] memory G;
      G[0] = Gx;
      G[1] = Gy;
      uint[2] memory xGG;
  ...
  // We verify the 1-out-of-k ZKP for Borda count protocol.

function verify1outofkZKP(uint[12] ab, uint[12] xGyG, uint[6] params, uint comp ) returns (bool) {
      uint k;      
      uint[2] memory temp1;
      uint[3] memory temp2;
      uint[3] memory temp3;
      ...
      uint[2][3] memory yG = voters[i[0]].reconstructedkey; 
      uint[2][3] memory xG = voters[i[0]].registeredkey;    
      ...
      for (k=0; k<3; k++) {
          ...
          // Does c =? d1 + d2 (mod n)
          if(uint(sha256(msg.sender, xGyG, ab)) != addmod(params[0],addmod(params[1],params[2],nn),nn)) {
              return false;
          }
              if ((3 - comp) == 2) {   
                  temp3[0] = G[0];    
                  ...
                  temp3 = Secp256k1._mul((uint)(3 - comp), G);  
              }  
          ...
          for (k=0; k<3; k++) {
              // a1 =? g^{r1} * x^{d1}
              temp2 = Secp256k1._mul(params[k + 3], G); 
              ...
              temp1[0] = temp3[0];
              temp1[1] = temp3[1];
              temp2 = Secp256k1._mul(params[k + 3],yG[k]);
