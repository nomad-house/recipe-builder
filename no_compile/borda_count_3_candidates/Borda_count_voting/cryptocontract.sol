...
contract AnonymousCryptoContract {
  ...
  // Retrieve the commitment hash for a voters vote.
   ...
   function commitToVote(uint[18] params, uint[6] yG, uint[12] xGyG, uint[36] ab) returns (bytes32[4] h) {
    uint i=0;
    uint j=0;
    uint[6] memory temp1;
    uint[12] memory temp2;
    h[0] =  sha256(msg.sender, yG, xGyG);
    for (i=0; i<3; i++) {
        for (j=0; j <6; j++) {
            temp1[j] = params[i*6 + j];
        }
        for (j=0; j<12; j++) {
            temp2[j] = ab[i*12 + j];
        }
  ...
  // r = v - xz (mod p);
  // return(r,vG)
  function createZKP(uint[3] x, uint[3] v, uint[2][3] xG) returns (uint[4][3] res) {
  ...
  function verifyZKP(uint[2][3] xG/*uint[10] xG*/, uint[3] r, uint[3][3] vG/*uint[15] vG*/) returns (bool){
      uint[2] memory G;
      G[0] = Gx;
      G[1] = Gy;
      uint[2] memory xGG;
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPVote(uint[6] xG, uint[6] yG, uint[12] xGyG, uint[3] w, uint[3] r1, uint[3] d1, uint[3] x, uint[4] choice_all) returns (uint[12] res, uint[6] res2) {
      uint i=0;
      uint j=0;
      uint[12] memory k;
      uint[2] memory temp2;
      ...
      if (i == 2) {
          ...
          temp2[0] = G[0];
          temp2[1] = pp - G[1];
          temp1 = Secp256k1_noconflict._mul((uint)(3-i),temp2);
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPv1Vote(uint[6] xG, uint[6] yG, uint[3] w, uint[3] r1, uint[3] d1, uint[3] x, uint[4] choice_all) returns (uint[12] res, uint[12] xGyG,  uint[6] res2) {
      uint i=0;
      uint j=0;
      uint[12] memory k;
      uint[2] memory temp2;
      ...
      temp2[0] = G[0];
      temp2[1] = pp - G[1];
      temp1 = Secp256k1_noconflict._mul(3,temp2);
      ECCMath_noconflict.toZ1(temp1, pp);
      k[2] = temp1[0];
      k[3] = temp1[1];
