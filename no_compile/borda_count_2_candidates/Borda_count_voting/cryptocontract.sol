...
contract AnonymousCryptoContract {
  ...
  // Retrieve the commitment hash for a voters vote.
   ...
   function commitToVote(uint[8] params, uint[4] yG, uint[8] xGyG, uint[16] ab) returns (bytes32[3] h) {
    uint i=0;
    uint j=0;
    uint[4] memory temp1;
    uint[8] memory temp2;
  ...
  // r = v - xz (mod p);
  // return(r,vG)
  function createZKP(uint[2] x, uint[2] v, uint[2][2] xG) returns (uint[4][2] res) {
  ...
  function verifyZKP(uint[2][2] xG, uint[2] r, uint[3][2] vG) returns (bool){
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPVote(uint[4] xG, uint[4] yG, uint[8] xGyG, uint[2] w, uint[2] r1, uint[2] d1, uint[2] x, uint[3] choice_all) returns (uint[8] res, uint[4] res2) {
      uint i=0;
      uint j=0;
      uint[8] memory k;
      uint[2] memory temp2;
   ...
   // random 'w', 'r1', 'd1'
  function create1outofkZKPv1Vote(uint[4] xG, uint[4] yG, uint[2] w, uint[2] r1, uint[2] d1, uint[2] x, uint[3] choice_all) returns (uint[8] res, uint[8] xGyG,  uint[4] res2) {
      uint i=0;
      uint j=0;
      uint[8] memory k;
      uint[2] memory temp2;
      ...
      temp1[1] = pp - G[1];
      temp1[2] = 1;
      temp1 = Secp256k1_noconflict._double(temp1);
      ECCMath_noconflict.toZ1(temp1, pp);
      k[2] = temp1[0];
      k[3] = temp1[1];
