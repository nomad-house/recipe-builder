...
contract AnonymousVoteContract is owned {
  ...
  struct Voter {
      address addr;
      uint[2][2] registeredkey;
      uint[2][2] reconstructedkey;
      bytes32[3] commitment;
      uint[2][2] vote;
      uint       sendtrx;
  }

  // Work around function to fetch details about a voter
  function getVoter() returns (/*uint[2][5] _registeredkey, uint[2][5] _reconstructedkey,*/uint[4] _registeredkey, uint[4] _reconstructedkey, bytes32[3] _commitment){
  ...
  uint[2][2] public finaltally; // Final tally
  ...
  function deadlinePassed() returns (bool){
      ...
      if(state == State.FINISHED && msg.sender == owner && refunds[owner] == 0 && (block.timestamp > endRefundPhase || totaltorefund == totalrefunded)) {
         ...
         uint[2][2] memory empty;
         bytes32[3] memory empty1;
  ...
  function register(uint[2][2] xG, uint[3][2] vG, uint[2] r) inState(State.REGISTRATION) payable returns (bool) {
   ...
   */
  function submitCommitment(bytes32[3] h) inState(State.COMMITMENT) {
...
function submitVote(uint[8] ab, uint[8] xGyG, uint[4] params, uint comp) inState(State.VOTE) returns (bool) {
     // HARD DEADLINE
  ...
  function verifyZKP(uint[2][2] xG, uint[2] r, uint[3][2] vG) returns (bool){
      uint[2] memory G;
      G[0] = Gx;
      G[1] = Gy;
      uint[2] memory xGG;
  ...
  // We verify the 1-out-of-k ZKP used in the Borda count protocol.
function verify1outofkZKP(uint[8] ab, uint[8] xGyG, uint[4] params, uint comp ) returns (bool) {
      uint k;      
      uint[2] memory temp1;
      uint[3] memory temp2;
      uint[3] memory temp3;
      ...
      uint[2][2] memory yG = voters[i[0]].reconstructedkey; 
      uint[2][2] memory xG = voters[i[0]].registeredkey;    
      ...
      for (k=0; k<2; k++) {
          ...
          // Does c =? d1 + d2 (mod n)
          if(uint(sha256(msg.sender, xGyG, ab)) != addmod(params[0],params[1],nn)) {
              return false;
          }
              if ((2 - comp) == 2) {   
                  temp3[0] = G[0];    
                  ...
                  temp3 = Secp256k1._mul((uint)(2 - comp), G);  
              } 
          ...
          for (k=0; k<2; k++) {
              // a1 =? g^{r1} * x^{d1}
              temp2 = Secp256k1._mul(params[k + 2], G);  
              ...
              temp1[0] = temp3[0];
              temp1[1] = temp3[1];
              temp2 = Secp256k1._mul(params[k + 2],yG[k]);
